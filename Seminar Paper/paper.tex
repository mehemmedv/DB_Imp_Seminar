...
give info about results. 
 
 \subsubsection*{Light Update}
 As it is shown from the results of \textbf{\textit{Simple Update}}, it is the most time consuming operation for CSR format. A new update structure -\textbf{\textit{Light Update}}- is a fast update method which is up to 13x faster than the previous update.
 \\
 
 results\\
 
 give some info about the result \\
 ...
 I evaulate the system in a mixed workload in which every 20th operation of queries is an update query which adds a new edge to the graph. Therefore, it gives us an overview of how good CSR is really on temporal graphs.
